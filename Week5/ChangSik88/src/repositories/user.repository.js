...
export const addUser = async (data) => {
  ...
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO member (email, name, gender, age, address, spec_address, phone_num, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?);`,
      [
        data.email,
        data.name,
        data.gender,
  ...
  try {
    const [preferences] = await pool.query(
      "SELECT mp.id, mp.category_id, mp.member_id, fcl.name " +
        "FROM member_prefer mp JOIN food_category fcl on mp.category_id = fcl.id " +
